...
class comlaporan_klasifikasi_tahun{
	function __construct(){
		$this->CI = & get_instance();
		$params = new stdClass;
		$params->lib['com_url'] = 'admin/com/laporan_klasifikasi/';
		$params->lib['class_name'] = 'comlaporan_klasifikasi_tahun';
		$params->lib['header_caption'] = '';
		$this->CI->load->com($params->lib['class_name'],'model',array('name'=>'Laporanklasifikasitahun_model','alias'=>'com_model'));
	...
	function index(){
		$this->content['header_caption'] = 'Laporan &raquo; Rekapitulasi Data Arsip &raquo; Laporan Kode Klasifikasi dan Tahun Arsip';
		...
		$this->content_default['segments']['head'] = $this->CI->load->com('comlaporan_klasifikasi_tahun','view',array('name'=>'default','data'=>@$this->content ,'return'=>false));		
	}	
	...
	{	
		...
		$this->CI->myexcel_klasifikasi->getActiveSheet()->getColumnDimension('B')->setWidth(14.25);		
...
/*
		$this->CI->myexcel_klasifikasi->writeRow('A5:C5', 'Klasifikasi');		
		$this->CI->myexcel_klasifikasi->writeRow('D5', ':');		
		$this->CI->myexcel_klasifikasi->writeRow('E5:F5', ($klasifikasi=="")?'ALL':$klasifikasi);
		
		$this->CI->myexcel_klasifikasi->writeRow('H5:H5', 'Tahun :');		
		$this->CI->myexcel_klasifikasi->writeRow('I5:I5', ($tahun=="")?'ALL':$tahun);
		*/
		$this->CI->myexcel_klasifikasi->writeRow('A6:A7', 'DEPO');		
		$this->CI->myexcel_klasifikasi->writeRow('B6:B7', 'TAHUN');		
		$this->CI->myexcel_klasifikasi->writeRow('C6:G7', 'KLASIFIKASI');		
	...
	//	foreach($data_depo as $row){
		if($depo==""){
			...
			$this->CI->myexcel_klasifikasi->writeRow('B'.$loop.':'.'B'.$loop, ($tahun=="")?"ALL":$tahun);	
			$this->CI->myexcel_klasifikasi->getActiveSheet()->getStyle('B'.$loop.':'.'B'.($colom_depo+$loop))->applyFromArray(array('alignment' => array('horizontal' => 'center', 'vertical'  => 'center')));			
			
			$this->CI->myexcel_klasifikasi->getActiveSheet()->mergeCells('A'.$loop.':'.'A'.($colom_depo+$loop));
			$this->CI->myexcel_klasifikasi->getActiveSheet()->mergeCells('B'.$loop.':'.'B'.($colom_depo+$loop));		
			$this->CI->myexcel_klasifikasi->getActiveSheet()->getStyle('A'.$loop.':'.'A20')->applyFromArray($style_data_kolom);
			foreach(@$data as $rowdata){
				$jml_data = $this->CI->com_model->get_data_laporan_jml($depo, ($klasifikasi=='')?$rowdata['id_kode_masalah']:$klasifikasi, $bulan, $tahun); 
				$this->CI->myexcel_klasifikasi->writeRow('C'.$loop.':'.'G'.$loop, $rowdata['kode']);		
