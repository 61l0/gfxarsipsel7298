...
class Piliharsip extends Grid {
	function __construct($config=array()){
	    // ===============================
		$this->CI = & get_instance();
		$params = new stdClass;
		$params->lib['class_name'] = 'piliharsip';
		...
		if( !empty($_POST['term']) )
		{
			...
			$this->CI->session->set_userdata('p_term',$_POST['term']);
		}
	...
	// function index_segments($params=false){
	     ...
	     // $this->content_default['segments']['before_script'] = $this->CI->load->com($this->content['master_class_name'].'/subcom/piliharsip','view',array('name'=>'before_script','data'=>$this->content ,'return'=>true));
    // }
	...
	function comjs_features($params=false){
		...
		$this->content['comjs_features']['comjs_extra'] = $this->CI->load->com($this->content['master_class_name'].'/subcom/piliharsip','view',$conf_view_features);
		$conf_view_features = array(
			'name'=>'comjs_gridcomplete',
			'data'=>$this->content,
			'return'=>true
		);
		$this->content['comjs_features']['gridcomplete'] = $this->CI->load->com($this->content['master_class_name'].'/subcom/piliharsip','view',$conf_view_features);		
