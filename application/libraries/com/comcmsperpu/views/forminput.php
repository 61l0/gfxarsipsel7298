 <?=form_open($com_url.'formaction',' id="myForm"');?>
	...
	<input type="hidden" id="id_peraturan_perundangan" name="id_peraturan_perundangan" value="<?=@$id_peraturan_perundangan?>">
	 <div id="notification-peraturan_perundangan" class="notification-error" style="background:#CC6600"></div>
	<table class="table-flat" width="100%" height="100%">
		...
		<tr>
			<td>File</td>
			<td>
				: <div id="peraturan_perundangan-uploader" class="fm-button ui-state-default ui-corner-all fm-button-icon-left">
...
<? 
				...
				if(is_file($path)): 
					...
					$image_thumb .= '<span id="file_'.@$id_peraturan_perundangan.'"> '.@$data->file;
					$image_thumb .= ' <a onclick="'.$class_name.'grid.extra.remove_file({id_peraturan_perundangan:'.@$id_peraturan_perundangan.'}); '.$class_name.'grid.extra.loadFormImageEditingGaleri('.@$id_peraturan_perundangan.'); return false;" class="fm-button ui-state-default ui-corner-all fm-button-icon-left"><span class="ui-icon ui-icon-trash"></span>Hapus</a></span>';
				endif;
?>
				<?=@$image_thumb;?>
				  </span>
...
<script language="javascript">	 
	jQuery(document).ready(function() {
		...
		jQuery(function(){	
			var btnUpload=$('#peraturan_perundangan-uploader') , interval;
			var status=$('#notification-peraturan_perundangan');	
			new AjaxUpload(btnUpload, {
				action: '<?=site_url($com_url);?>/saveupload',
				name: 'peraturan_perundangan_file',
				data: {id_peraturan_perundangan:<?=@$id_peraturan_perundangan;?>},
				...
				onComplete: function(file, response){
					...
					if(arr_result[0]=="success"){
						jQuery('#id_peraturan_perundangan').val(arr_result[2]);
