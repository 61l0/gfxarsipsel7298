...
jQuery(document).ready(function() { 
	...
	jQuery(function(){	
		...
		new AjaxUpload(btnUpload, {
			action: '<?=site_url('admin/com/perusahaan/subfn/comproduk/saveupload');?>',
			name: 'img_file',
	...
	function loadFormImageEditingGaleri(){
		loadFragment('#table_form','<?=site_url('admin/com/perusahaan/subfn/comproduk/formeditingimage');?>',{id_produk:<?=@$id_produk;?>});
	} 
	
});
	function Remove_Image(id){
		jQuery.post('<?=site_url('admin/com/perusahaan/subfn/comproduk/removeimage');?>', {id_produk_gambar: id},function(data){
			var status= $('#notif-imggaleri');
			if(data=='sukses'){
				status.html('');
				status.removeClass('msg-box-false');
    ...
    <tr>
      <td colspan="2">
          ...
          <span>Click tombol disamping untuk mengupload gambar-gambar terkait dengan komoditi, anda dapat mengupload lebih dari satu file.</span>
			...
			<form id="imgeditingGaleri-form" method="post" action="<?=site_url('admin/com/perusahaan/subfn/comproduk');?>/saveimagedata" enctype="multipart/form-data">
			<input type="hidden" name="id_produk" id="id_produk" value="<?=@$id_produk;?>"  />
			  ...
			  <tr><td colspan="2"><input type="submit" name="save_data" value="Simpan Perubahan" />&nbsp;
			   ...
			   <? foreach($data as $rowx){
					...
					?>
					<tr id="tr_<?=@$rowx->id_produk_gambar?>"><td width="25%" style="border-bottom:1px solid #C0D6ED"><?=@$image_thumb?></td>
						...
						Tanggal <input type="text" size="10" name="tanggal_<?=@$rowx->id_produk_gambar?>" id="tanggal_<?=@$rowx->id_produk_gambar?>" value="<?=@$rowx->tgl_buat?>" class="dpicker-galerifoto" readonly="readonly" disabled/>
						</td>
					</tr>
				<?
					}
