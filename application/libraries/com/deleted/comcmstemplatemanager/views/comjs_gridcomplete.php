<?=$class_name;?>grid.opt.gridComplete = function(){
	...
	for(var i=0;i < ids.length;i++){
		...
		var fnNameSv = "<?=$class_name;?>grid.btn_grid_view({id_template:'"+cl+"'});";
		...
		var fnName3 = "<?=$class_name;?>grid.btn_grid_edit({id_template:"+cl+",oper:'edit'});";
		...
		var fnDel = "<?=$class_name;?>grid.extra.del({id_template:"+cl+"});";
		...
		var fnref = "<?=$class_name;?>grid.extra.refresh({id_template:"+cl+"});";
		var ref = jQuery("<span />").addClass("ui-icon ui-icon-refresh").attr('onclick',fnref);
		var insref = jQuery("<div />").addClass("ui-pg-div ui-inline-delete").css("float","left").css("cursor","pointer").attr("title","Klik untuk mengubah status");
		inssv.append(sv);
		ins3.append(sp3);
		insdel.append(del);
		insref.append(ref);
