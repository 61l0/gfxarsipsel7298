...
class Compemusnahan extends Grid{
	function __construct(){
		...
		if($status == 'all' || !$status){
			...
			$params->model['query']['query_filter']=array(
				'inaktif_sampai'=>array(
					'type'=>'where'
				,'field'=>"a.inaktif_sampai < ".$date." and  a.status IS NULL" 
					),
			);
		}
	...
	function formaction_tinjau()
	{
		...
		if($oper == 'tinjau'){
			$hasil= $this->CI->model_pemusnahan->tinjau($id_data,$id_retensi);
			echo json_encode($hasil);
		}else{
	...
	function griddata(){
		...
		//		echo strtotime($grid_data->rows[$key]->inaktif_sampai . ' 00:00:00');
			...
			*/
			...
			$grid_data->rows[$key]->rt_aktif =  displayMasa($diffAktif );
			$grid_data->rows[$key]->rt_inaktif =  displayMasa($diffInaktif);

			$grid_data->rows[$key]->rt_desc = strtoupper(preg_replace('/_/g', ' ', $grid_data->rows[$key]->rt_desc ) );
		}
