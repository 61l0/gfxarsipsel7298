...
class comlap_surat_keluar{
	function __construct(){
		$this->CI = & get_instance();
		$params = new stdClass;
		$params->lib['com_url'] = 'admin/com/lap_surat_keluar/';
		$params->lib['class_name'] = 'comlap_surat_keluar';
		
		$this->CI->load->com($params->lib['class_name'],'model',array('name'=>'lap_surat_keluar_model','alias'=>'com_model'));
	...
	function index(){
		$this->content['header_caption'] = 'Laporan &raquo; Surat &raquo; Surat Keluar';
		...
		$this->content_default['segments']['head'] = $this->CI->load->com('comlap_surat_keluar','view',array('name'=>'default','data'=>@$this->content ,'return'=>false));		
	}	

	function rptsuratkeluar()
	{	
		...
		$data['no'] 			= 1;			
		$data['surat_keluar_list'] 	= $surat_masuk_list;
		$count 	= count($retensi_list );
		$html 	= $this->CI->load->com('comlap_surat_keluar','view',array('name'=>'excel_surat_keluar','data'=>$data ,'return'=>true));

		require_once(CLASSES_PATH.'PHPExcel.php');		
		require_once(APPPATH.'libraries/HtmlExcel.php');

		$token = md5('comlap_surat_keluar'.$_SESSION['user_id'] . $this->browser->user_agent . date('Y-m-d'));
