<?=$class_name;?>grid.opt.gridComplete = function(){
	...
	for(var i=0;i < ids.length;i++){
		...
		var fnNameSv = "view({id_daftar_istilah:'"+cl+"'});";
		...
		var fnName3 = "<?=$class_name;?>grid.btn_grid_edit({id_daftar_istilah:"+cl+",oper:'edit'});";
		...
		var fnref = "<?=$class_name;?>grid.extra.act({id_daftar_istilah:"+cl+",oper:'refresh'});";
		var ref = jQuery("<span />").addClass("ui-icon ui-icon-refresh").attr('onclick',fnref);
		var insref = jQuery("<div />").addClass("ui-pg-div ui-inline-refresh").css("float","left").css("cursor","pointer").attr("title","Klik untuk menghapus data");

		var fnDel = "<?=$class_name;?>grid.extra.act({id_daftar_istilah:"+cl+",oper:'del'});";
