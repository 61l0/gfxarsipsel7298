<?=$class_name;?>grid.opt.gridComplete = function(){
	if( $('div.toolbar-search').is(':visible') && $('div.toolbar-search > input#cari').val().length )	
	{
		var type = $('select#pilih_cari option:selected').text();
		var keyword= $('div.toolbar-search > input#cari').val();
		var total = $('#gridcompengolahanPager_right > .ui-paging-info').text().split(' of ')[1].replace(/\s/,'');
		var info = 'Pencarian berdasarkan ' + type + ' dengan kata kunci &quot;'+ keyword +'&quot; menghasilkan <b>'+ total+'</b> item. ';
		console.log(info);
		$('div.search-info').html(info).show();
	}
	else
	{
		$('div.search-info').hide();
	}
