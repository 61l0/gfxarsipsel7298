<?=$class_name;?>grid.opt.gridComplete = function(){
	...
	for(var i=0;i < ids.length;i++){
		...
		if(row.type == 'folder')
			row.type_text='sampul';
		else
			row.type_text = row.type;
		jQuery("#"+<?=$class_name;?>grid.id).jqGrid('setRowData',cl,row);
		if(row.act == ""){
			var fnAdd = "<?=$class_name;?>grid.btn_grid_plus({id_parent:"+cl+",act:'ins'});commstlokasisimpangrid.MODE=1;commstlokasisimpangrid.ID_PARENT="+cl+";";
			...
			var fnEdit = "<?=$class_name;?>grid.btn_grid_edit({id_row:"+cl+",act:'edit'});commstlokasisimpangrid.MODE=2;";
